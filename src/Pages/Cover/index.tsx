...
const Cover: FC<CoverProps> = (props) => {
  ...
  const [leaveCover, setLeaveCover] = useState(false);
  ...
  useEffect(() => {
    ...
    intervalTimer = setInterval(() => {
      ...
      for (let i = 0; i < codeDtls.length; i++) {
        ...
        ctx.shadowOffsetY = 1;
        codeDtls[i].strIdx += 1;
        let showAwaitVerticalLine =
          codeDtls[i].strIdx < codes[codeDtls[i].codeIdx].length;
        ctx.fillText(
          `${codes[codeDtls[i].codeIdx].substring(0, codeDtls[i].strIdx)} ${
            showAwaitVerticalLine ? "|" : ""
          }`,
          codeDtls[i].x,
          codeDtls[i].y
        );
      ...
      >
        ...
        <Grid container item xs={12} position="absolute" zIndex={2}>
          <Grid
            ...
            justifyContent="center"
          >
            <Typography fontSize={["18px", "36px"]}>Hey, I am</Typography>
            <Box
